...
def Model_Training(final_array,final_array_Y):
    ...
    model.fit(X_train, y_train, epochs = 10, batch_size = 32,validation_data=(X_val, y_val))#,callbacks=[monitor]) #训练模型1000次
    '''
    d = 0.01
    model = Sequential()
    ...
    print(model.summary())
    with open('./IMU_LSTM_Model_Performance/Test2/modelsummary.txt', 'w') as f:
    
        model.summary(print_fn=lambda x: f.write(x + '\n'))
    
    #保存模型和權重
    saved = model.save('./IMU_LSTM_Model_Performance/Test2/model.h5')    
    model.save_weights('./IMU_LSTM_Model_Performance/Test2/model_weights.h5')  # to store
    ...
    plt.legend()
    plt.show()
    fig.savefig('./IMU_LSTM_Model_Performance/Test1/train_Validation_loss.png')
    ...
    plt.legend()
    plt.show()
    fig1.savefig('./IMU_LSTM_Model_Performance/Test1/train_Validation_accuracy.png')
    '''
...
if __name__ == '__main__':
   ...
   print("Finish")
   X_train,X_val,X_test =  Model_Training(final_array,final_array_YYY)         
