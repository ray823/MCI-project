...
def Model_Training(final_array,final_array_Y):
    ...
    print(model.summary())
    with open('./IMU_LSTM_Model_Performance/Test9/modelsummary.txt', 'w') as f:
    
        model.summary(print_fn=lambda x: f.write(x + '\n'))
    
    #保存模型和權重
    saved = model.save('./IMU_LSTM_Model_Performance/Test9/model.h5')    
    model.save_weights('./IMU_LSTM_Model_Performance/Test9/model_weights.h5')  # to store
    ...
    plt.ylabel('Value')
    # 保存圖形到文件（以PNG格式保存）
    plt.savefig('./IMU_LSTM_Model_Performance/Test9/loss_acc_curves.png')
    ...
    plt.legend()
    plt.show()
    fig.savefig('./IMU_LSTM_Model_Performance/Test9/train_Validation_loss.png')
    ...
    plt.legend()
    plt.show()
    fig1.savefig('./IMU_LSTM_Model_Performance/Test9/train_Validation_accuracy.png')
